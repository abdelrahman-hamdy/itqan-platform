...
class SupervisorDashboardController extends BaseSupervisorWebController
{
    public function index($subdomain = null): View
    {
        ...
        // ====================================================================
        // General Stats — scoped to assigned teachers for supervisors, academy-wide for admins
        // ====================================================================
        if ($isAdmin) {
            $generalStats = Cache::remember("supervisor_general_stats_{$academyId}", 60, function () use ($academyId) {
                ...
                return compact(
                    'totalUsers', 'activeUsers', 'totalIncome',
                    'totalSessions', 'passedSessions',
                    'totalStudents', 'totalQuranTeachers', 'totalAcademicTeachers', 'totalParents', 'totalSupervisors'
                );
            });
        } else {
            // Supervisor: scope stats to assigned teachers
            $totalQuranTeachers = count($quranTeacherIds);
            $academicTeacherUserIds = $this->getAssignedAcademicTeacherIds();
            $totalAcademicTeachers = count($academicTeacherUserIds);

            $totalQuranSessions = ! empty($quranTeacherIds) ? QuranSession::whereIn('quran_teacher_id', $quranTeacherIds)->count() : 0;
            $totalAcademicSessions = ! empty($academicTeacherProfileIds) ? AcademicSession::whereIn('academic_teacher_id', $academicTeacherProfileIds)->count() : 0;
            $totalSessions = $totalQuranSessions + $totalAcademicSessions;

            $passedSessions = (! empty($quranTeacherIds) ? QuranSession::whereIn('quran_teacher_id', $quranTeacherIds)->where('scheduled_at', '<', now())->count() : 0)
                + (! empty($academicTeacherProfileIds) ? AcademicSession::whereIn('academic_teacher_id', $academicTeacherProfileIds)->where('scheduled_at', '<', now())->count() : 0);

            $generalStats = [
                'totalUsers' => $totalQuranTeachers + $totalAcademicTeachers,
                'activeUsers' => $totalQuranTeachers + $totalAcademicTeachers,
                'totalIncome' => 0,
                'totalSessions' => $totalSessions,
                'passedSessions' => $passedSessions,
                'totalStudents' => 0,
                'totalQuranTeachers' => $totalQuranTeachers,
                'totalAcademicTeachers' => $totalAcademicTeachers,
                'totalParents' => 0,
                'totalSupervisors' => 0,
            ];
        }
        ...
        if ($isAdmin) {
            $monthQuranSessions = QuranSession::where('academy_id', $academyId)->whereMonth('scheduled_at', now()->month)->whereYear('scheduled_at', now()->year)->count();
            $monthAcademicSessions = AcademicSession::where('academy_id', $academyId)->whereMonth('scheduled_at', now()->month)->whereYear('scheduled_at', now()->year)->count();
            $monthInteractiveSessions = InteractiveCourseSession::whereHas('course', fn ($q) => $q->where('academy_id', $academyId))->whereMonth('scheduled_at', now()->month)->whereYear('scheduled_at', now()->year)->count();
        } else {
            $monthQuranSessions = ! empty($quranTeacherIds) ? QuranSession::whereIn('quran_teacher_id', $quranTeacherIds)->whereMonth('scheduled_at', now()->month)->whereYear('scheduled_at', now()->year)->count() : 0;
            $monthAcademicSessions = ! empty($academicTeacherProfileIds) ? AcademicSession::whereIn('academic_teacher_id', $academicTeacherProfileIds)->whereMonth('scheduled_at', now()->month)->whereYear('scheduled_at', now()->year)->count() : 0;
            $monthInteractiveSessions = 0;
        }
        $monthSessions = $monthQuranSessions + $monthAcademicSessions + $monthInteractiveSessions;

        // Revenue stats — admin only (hidden in blade for supervisors)
        $thisMonthRevenue = $isAdmin ? Payment::where('academy_id', $academyId)->where('status', PaymentStatus::COMPLETED->value)
            ->whereMonth('created_at', now()->month)->whereYear('created_at', now()->year)->sum('amount') : 0;
        $revenueGrowth = 0;
        if ($isAdmin) {
