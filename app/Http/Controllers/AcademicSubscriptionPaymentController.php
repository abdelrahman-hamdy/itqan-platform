...
/**
 * Handles "Pay Now" for pending academic subscriptions.
 *
...
class AcademicSubscriptionPaymentController extends Controller
{
    ...
    public function create(Request $request, $subdomain, $subscriptionId): View|RedirectResponse
    {
        ...
        return view('payments.academic-subscription', compact('academy', 'subscription'));
    }
     ...
     */
    private function processAndRedirect(Academy $academy, AcademicSubscription $subscription, string $gateway): RedirectResponse
    {
        $user = Auth::user();
        ...
        try {
            ...
            $finalPrice = $pendingCycle?->final_price
                ?? $subscription->final_price
                ?? $subscription->monthly_price;

            // Check for an existing pending payment with a valid gateway URL (idempotency)
            $existingPayment = Payment::forPayable(AcademicSubscription::class, $subscription->id)
    ...
    {
        $sub = AcademicSubscription::where('academy_id', $academy->id)
