    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                ...
                Forms\Components\Section::make('نوع الباقة')
                    ->description('تحديد نوع الباقة التعليمية')
                    ->schema([
                        Forms\Components\Grid::make(2)
                            ->schema([
                                Forms\Components\Select::make('package_type')
                                    ...
                                    ->afterStateUpdated(function (callable $set, $state) {
                                        if ($state === 'individual') {
                                            $set('max_students_per_session', 1);
                                        } else {
                                            $set('max_students_per_session', 6);
                                        }
                                    }),
                                ...
                                Forms\Components\TextInput::make('max_students_per_session')
                                    ...
                                    ->maxValue(10)
                                    ->default(1)
                                    ->visible(fn (callable $get) => $get('package_type') === 'group'),
                            ]),
                    ]),
            ...
            ->schema([
                Components\Section::make('معلومات الباقة')
                    ->schema([
                        ...
                        Components\TextEntry::make('package_type')
                            ->label('نوع الباقة')
                            ->formatStateUsing(fn (string $state): string => match($state) {
                                'individual' => 'دروس فردية',
                                'group' => 'دروس جماعية',
