...
class StudentPerformanceChartWidget extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        $startDate = match ($this->filter) {
            'week' => now()->subWeek(),
            'month' => now()->subMonth(),
            'all' => now()->subYear(), // Last year for "all"
        ...
        for ($i = $intervals - 1; $i >= 0; $i--) {
            ...
            $periodReports = AcademicSessionReport::whereIn('session_id', $teacherSessionIds)
                ->whereBetween('created_at', [$periodStart, $periodEnd])
                ->get();

            // Calculate average homework grade (0-10 scale, convert to percentage)
            $avgHomework = $periodReports->whereNotNull('homework_degree')->avg('homework_degree');
            $homeworkGrades[] = $avgHomework !== null ? round($avgHomework * 10, 1) : 0;
