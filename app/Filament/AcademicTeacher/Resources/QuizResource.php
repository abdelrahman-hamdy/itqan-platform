                            ...
                            ->options(function (Forms\Get $get) {
                                ...
                                if ($type === AcademicSubscription::class) {
                                    return AcademicSubscription::where('teacher_id', $teacherId)
                                        ->with('student')
                                        ->get()
                                        ->mapWithKeys(fn ($s) => [$s->id => ($s->student?->first_name ?? '') . ' ' . ($s->student?->last_name ?? '') . ' - ' . ($s->subject_name ?? 'درس خاص')]);
                                } else {
                                    return InteractiveCourse::where('assigned_teacher_id', $teacherId)->pluck('title', 'id');
                                }
                            })
                            ->required()
                            ->searchable(),
