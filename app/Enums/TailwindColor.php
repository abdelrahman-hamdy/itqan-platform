...
enum TailwindColor: string
{
    ...
    /**
     * Get the Tailwind color class for a specific shade
     */
    public function getColorClass(int $shade = 500): string
    {
        return "{$this->value}-{$shade}";
    }

    /**
     * Get the hex color value for a specific shade
     */
    public function getHexValue(int $shade = 500): string
    {
        ...
        return $colors[$this->value][$shade] ?? $colors[$this->value][500];
    }
     ...
     */
    public function getCssVariableName(string $type = 'primary'): string
    {
        return "--color-{$type}";
    }
     ...
     */
    public function generateCssVariables(string $type = 'primary'): array
    {
        $variables = [];
        $shades = [50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950];

        foreach ($shades as $shade) {
            $variables["--color-{$type}-{$shade}"] = $this->getHexValue($shade);
        }
