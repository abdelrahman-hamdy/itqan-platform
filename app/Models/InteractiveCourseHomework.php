<?php

namespace App\Models;

use App\Enums\HomeworkStatus;
use App\Models\Traits\ScopedToAcademy;
use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Relations\BelongsTo;
use Illuminate\Database\Eloquent\Relations\HasMany;
use Illuminate\Database\Eloquent\SoftDeletes;

/**
 * InteractiveCourseHomework
 *
 * Represents a homework assignment for an interactive course session.
 * Aligned with AcademicHomework structure for consistency.
 *
 * One assignment can have multiple student submissions (InteractiveCourseHomeworkSubmission).
 */
class InteractiveCourseHomework extends Model
{
    use HasFactory, ScopedToAcademy, SoftDeletes;

    protected $table = 'interactive_course_homework';

    protected $fillable = [
        'academy_id',
        'interactive_course_session_id',
        'teacher_id',
        'title',
        'description',
        'instructions',
        'teacher_files',
        'due_date',
        'allow_late_submissions',
        'max_score',
        'status',
        'is_active',
        'total_students',
        'submitted_count',
        'graded_count',
        'late_count',
        'average_score',
        'created_by',
        'updated_by',
    ];

    protected $casts = [
        'status' => HomeworkStatus::class,
        'teacher_files' => 'array',
        'due_date' => 'datetime',
        'max_score' => 'decimal:2',
        'average_score' => 'decimal:2',
        'allow_late_submissions' => 'boolean',
        'is_active' => 'boolean',
        'total_students' => 'integer',
        'submitted_count' => 'integer',
        'graded_count' => 'integer',
        'late_count' => 'integer',
    ];

    protected $attributes = [
        'status' => 'published',
        'is_active' => true,
        'allow_late_submissions' => true,
        'max_score' => 10,
        'total_students' => 0,
        'submitted_count' => 0,
        'graded_count' => 0,
        'late_count' => 0,
    ];

    /**
     * Relationships
     */
    public function academy(): BelongsTo
    {
        return $this->belongsTo(Academy::class);
    }

    public function session(): BelongsTo
    {
        return $this->belongsTo(InteractiveCourseSession::class, 'interactive_course_session_id');
    }

    public function teacher(): BelongsTo
    {
        return $this->belongsTo(User::class, 'teacher_id');
    }

    /**
     * Get homework submissions for this assignment
     */
    public function submissions(): HasMany
    {
        return $this->hasMany(InteractiveCourseHomeworkSubmission::class, 'interactive_course_homework_id');
    }

    public function creator(): BelongsTo
    {
        return $this->belongsTo(User::class, 'created_by');
    }

    public function updater(): BelongsTo
    {
        return $this->belongsTo(User::class, 'updated_by');
    }

    /**
     * Scopes
     */
    public function scopeForAcademy($query, int $academyId)
    {
        return $query->where('academy_id', $academyId);
    }

    public function scopeForSession($query, int $sessionId)
    {
        return $query->where('interactive_course_session_id', $sessionId);
    }

    public function scopeForTeacher($query, int $teacherId)
    {
        return $query->where('teacher_id', $teacherId);
    }

    public function scopeActive($query)
    {
        return $query->where('is_active', true)->where('status', HomeworkStatus::PUBLISHED->value);
    }

    public function scopePublished($query)
    {
        return $query->where('status', HomeworkStatus::PUBLISHED->value);
    }

    public function scopeOverdue($query)
    {
        return $query->where('due_date', '<', now())
            ->where('status', HomeworkStatus::PUBLISHED->value);
    }

    public function scopeDueSoon($query, int $days = 3)
    {
        return $query->whereBetween('due_date', [now(), now()->addDays($days)])
            ->where('status', HomeworkStatus::PUBLISHED->value);
    }

    public function scopeNeedsGrading($query)
    {
        return $query->where('submitted_count', '>', 'graded_count');
    }

    /**
     * Accessors
     */
    public function getIsOverdueAttribute(): bool
    {
        return $this->due_date && $this->due_date->isPast() && $this->status === HomeworkStatus::PUBLISHED;
    }

    public function getDaysUntilDueAttribute(): ?int
    {
        if (! $this->due_date) {
            return null;
        }

        return now()->diffInDays($this->due_date, false);
    }

    public function getSubmissionRateAttribute(): float
    {
        if ($this->total_students === 0) {
            return 0;
        }

        return round(($this->submitted_count / $this->total_students) * 100, 2);
    }

    public function getGradingProgressAttribute(): float
    {
        if ($this->submitted_count === 0) {
            return 0;
        }

        return round(($this->graded_count / $this->submitted_count) * 100, 2);
    }

    public function getPendingGradingCountAttribute(): int
    {
        return $this->submitted_count - $this->graded_count;
    }

    public function getStatusTextAttribute(): string
    {
        return match ($this->status) {
            HomeworkStatus::DRAFT => 'مسودة',
            HomeworkStatus::PUBLISHED => 'منشور',
            HomeworkStatus::IN_PROGRESS => 'قيد التنفيذ',
            HomeworkStatus::ARCHIVED => 'مؤرشف',
            default => $this->status->value ?? 'غير معروف',
        };
    }

    /**
     * Helper Methods
     */
    public function publish(): bool
    {
        if ($this->status !== HomeworkStatus::DRAFT) {
            return false;
        }

        $this->update([
            'status' => HomeworkStatus::PUBLISHED,
        ]);

        return true;
    }

    public function archive(): bool
    {
        $this->update(['status' => HomeworkStatus::ARCHIVED, 'is_active' => false]);

        return true;
    }

    public function updateStatistics(): void
    {
        $submissions = $this->submissions();

        $this->update([
            'submitted_count' => $submissions->whereIn('submission_status', ['submitted', 'late', 'graded'])->count(),
            'graded_count' => $submissions->where('submission_status', 'graded')->count(),
            'late_count' => $submissions->where('is_late', true)->count(),
            'average_score' => $submissions->whereNotNull('score')->avg('score'),
        ]);
    }

    /**
     * Get submission for a student
     */
    public function getSubmissionForStudent(int $studentId): ?InteractiveCourseHomeworkSubmission
    {
        return $this->submissions()->where('student_id', $studentId)->first();
    }

    public function hasSubmissionFrom(int $studentId): bool
    {
        return $this->submissions()
            ->where('student_id', $studentId)
            ->whereIn('submission_status', ['submitted', 'late', 'graded'])
            ->exists();
    }

    public function canBeSubmittedBy(int $studentId): bool
    {
        if ($this->status !== HomeworkStatus::PUBLISHED) {
            return false;
        }

        if (! $this->is_active) {
            return false;
        }

        // Check if already submitted
        if ($this->hasSubmissionFrom($studentId)) {
            return false;
        }

        // Check if overdue and late submissions not allowed
        if ($this->is_overdue && ! $this->allow_late_submissions) {
            return false;
        }

        return true;
    }

    /**
     * Static Methods
     */
    public static function createForSession(InteractiveCourseSession $session, array $data): self
    {
        $academyId = $session->academy_id ?? $session->course?->academy_id;

        return self::create(array_merge($data, [
            'academy_id' => $academyId,
            'interactive_course_session_id' => $session->id,
            'teacher_id' => $session->course?->assigned_teacher_id,
            'status' => HomeworkStatus::PUBLISHED,
        ]));
    }

    public static function getUpcomingForStudent(int $studentId, int $academyId, int $limit = 5)
    {
        return self::query()
            ->where('academy_id', $academyId)
            ->where('status', HomeworkStatus::PUBLISHED->value)
            ->where('is_active', true)
            ->whereHas('session.course.enrollments', function ($query) use ($studentId) {
                $query->where('student_id', $studentId);
            })
            ->whereDoesntHave('submissions', function ($query) use ($studentId) {
                $query->where('student_id', $studentId)
                    ->whereIn('submission_status', ['submitted', 'late', 'graded']);
            })
            ->orderBy('due_date', 'asc')
            ->limit($limit)
            ->get();
    }

    public static function getForTeacher(int $teacherId, int $academyId, ?string $status = null)
    {
        $query = self::query()
            ->where('academy_id', $academyId)
            ->where('teacher_id', $teacherId);

        if ($status) {
            $query->where('status', $status);
        }

        return $query->orderBy('due_date', 'desc')->get();
    }
}
