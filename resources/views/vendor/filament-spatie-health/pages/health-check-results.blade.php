    	...
    	@if (count($checkResults?->storedCheckResults ?? []))
    		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 mb-5">
    			@foreach ($checkResults->storedCheckResults as $result)
    				<div class="flex items-start gap-4 px-4 overflow-hidden py-5 text-opacity-0 transition transform bg-white shadow-sm shadow-gray-200 dark:shadow-black/25 dark:shadow-md dark:bg-gray-800 rounded-xl sm:p-6 md:min-h-[130px] dark:border-t dark:border-gray-700">
    					<div class="flex-shrink-0 flex justify-center items-center rounded-full p-2 {{ backgroundColor($result->status) }}">
