...
<x-filament-widgets::widget class="fi-wi-stats-overview grid gap-y-4">
    {{-- Desktop: show heading normally --}}
    @if ($hasHeading || $hasDescription)
        <div class="fi-wi-stats-overview-header grid gap-y-1 hidden md:grid">
            @if ($hasHeading)
    ...
    {{-- Desktop: show stats normally (always visible) --}}
    <div
        @if ($pollingInterval = $this->getPollingInterval())
            wire:poll.{{ $pollingInterval }}
        @endif
        @class([
            'fi-wi-stats-overview-stats-ctn grid gap-6 hidden md:grid',
            'md:grid-cols-1' => $columns === 1,
            'md:grid-cols-2' => $columns === 2,
            'md:grid-cols-3' => $columns === 3,
            'md:grid-cols-2 xl:grid-cols-4' => $columns === 4,
