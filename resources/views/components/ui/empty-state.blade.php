...
@php
    $variantClasses = match($variant) {
        'card' => "bg-white rounded-xl shadow-sm border border-gray-200 {$containerClass} text-center",
        'inline' => "bg-gray-50 rounded-xl {$containerClass} text-center",
        default => "text-center {$containerClass}",
    };
@endphp
        ...
        <div {{ $attributes->merge(['class' => $variantClasses]) }}>
            <div class="max-w-md mx-auto px-4">
                ...
                @if($actionUrl && $actionLabel)
                    <a href="{{ $actionUrl }}" class="inline-flex items-center px-6 py-3 min-h-[44px] {{ $buttonColorClass }} text-white rounded-xl transition font-medium">
                        @if($actionIcon)
