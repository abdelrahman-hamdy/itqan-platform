  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="hidden md:flex items-center justify-center h-20">
        <div class="flex items-center space-x-1 space-x-reverse">
          @if($academy->quran_enabled ?? true)
            <a href="{{ route('public.quran-circles.index', ['subdomain' => $academy->subdomain ?? 'test-academy']) }}" 
               class="flex items-center h-20 px-4 text-gray-700 hover:text-primary hover:bg-gray-50 transition-colors duration-200 focus:outline-none font-medium {{ request()->routeIs('public.quran-circles.*') ? 'text-primary bg-primary/5' : '' }}" 
               aria-label="حلقات القرآن الكريم">
              حلقات القرآن
            </a>
          @endif
          
          @if($academy->quran_enabled ?? true)
            <a href="{{ route('public.quran-teachers.index', ['subdomain' => $academy->subdomain ?? 'test-academy']) }}" 
               class="flex items-center h-20 px-4 text-gray-700 hover:text-primary hover:bg-gray-50 transition-colors duration-200 focus:outline-none font-medium {{ request()->routeIs('public.quran-teachers.*') ? 'text-primary bg-primary/5' : '' }}" 
               aria-label="معلمو القرآن الكريم">
              معلمو القرآن
            </a>
          @endif
          ...
          @if($academy->recorded_courses_enabled ?? true)
            <a href="{{ route('public.recorded-courses.index', ['subdomain' => $academy->subdomain ?? 'test-academy']) }}" 
               class="flex items-center h-20 px-4 text-gray-700 hover:text-primary hover:bg-gray-50 transition-colors duration-200 focus:outline-none font-medium {{ request()->routeIs('public.recorded-courses.*') ? 'text-primary bg-primary/5' : '' }}" 
               aria-label="الكورسات المسجلة">
              الكورسات المسجلة
            </a>
          @endif
        </div>
    ...
    <div class="md:hidden hidden" id="mobile-menu">
      <div class="px-2 pt-2 pb-3 space-y-1 bg-white border-t border-gray-200">
        @if($academy->quran_enabled ?? true)
          <a href="{{ route('public.quran-circles.index', ['subdomain' => $academy->subdomain ?? 'test-academy']) }}" class="block px-3 py-2 text-gray-700 hover:text-primary hover:bg-gray-50 rounded-md focus:outline-none" aria-label="حلقات القرآن الكريم">حلقات القرآن</a>
        @endif
        
        @if($academy->quran_enabled ?? true)
          <a href="{{ route('public.quran-teachers.index', ['subdomain' => $academy->subdomain ?? 'test-academy']) }}" class="block px-3 py-2 text-gray-700 hover:text-primary hover:bg-gray-50 rounded-md focus:outline-none" aria-label="معلمو القرآن الكريم">معلمو القرآن</a>
        @endif
        ...
        @if($academy->recorded_courses_enabled ?? true)
          <a href="{{ route('public.recorded-courses.index', ['subdomain' => $academy->subdomain ?? 'test-academy']) }}" class="block px-3 py-2 text-gray-700 hover:text-primary hover:bg-gray-50 rounded-md focus:outline-none" aria-label="الكورسات المسجلة">الكورسات المسجلة</a>
        @endif
