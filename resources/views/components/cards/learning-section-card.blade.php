  ...
  <div class="p-6 border-b border-gray-100">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center space-x-2 space-x-reverse">
        @if(isset($badge))
          <span class="px-2 py-1 text-xs font-medium rounded-full {{ $badgeColor ?? 'bg-green-100 text-green-800' }}">
            {{ $badge }}
          </span>
        @endif
      ...
      <div class="space-y-4">
        @foreach($items as $item)
          <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
            <div class="flex items-center space-x-3 space-x-reverse">
              <div class="w-10 h-10 rounded-lg flex items-center justify-center {{ $item['iconBgColor'] ?? 'bg-blue-100' }}">
                <i class="{{ $item['icon'] ?? 'ri-book-line' }} text-sm text-blue-600"></i>
              </div>
              <div class="flex-1">
