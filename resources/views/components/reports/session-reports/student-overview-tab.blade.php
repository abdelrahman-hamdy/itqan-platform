...
@php
    $typeOptions = [
        ...
        'quran_individual' => __('reports.type_quran_individual'),
        'quran_group' => __('reports.type_quran_group'),
        'academic' => __('reports.type_academic_lesson'),
    ...
    $typeBadgeConfig = [
        ...
        'quran_group' => ['class' => 'bg-emerald-100 text-emerald-700', 'text' => __('reports.type_quran_group'), 'icon' => 'ri-team-line', 'iconBg' => 'bg-gradient-to-br from-emerald-500 to-emerald-600'],
        'academic' => ['class' => 'bg-violet-100 text-violet-700', 'text' => __('reports.type_academic_lesson'), 'icon' => 'ri-graduation-cap-line', 'iconBg' => 'bg-gradient-to-br from-violet-500 to-violet-600'],
        'interactive' => ['class' => 'bg-purple-100 text-purple-700', 'text' => __('reports.type_interactive_course'), 'icon' => 'ri-live-line', 'iconBg' => 'bg-gradient-to-br from-purple-500 to-purple-600'],
    ];
@endphp
...
<div class="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 mb-4 md:mb-6">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-lg md:text-2xl font-bold text-gray-900">{{ $totalStudents }}</p>
                <p class="text-xs md:text-sm text-gray-600">{{ __('reports.total_students') }}</p>
            </div>
        </div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-lg md:text-2xl font-bold text-gray-900">{{ $avgAttendance . '%' }}</p>
                <p class="text-xs md:text-sm text-gray-600">{{ __('reports.avg_attendance') }}</p>
            </div>
        </div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-lg md:text-2xl font-bold text-gray-900">{{ $totalEntities }}</p>
                <p class="text-xs md:text-sm text-gray-600">{{ __('reports.total_entities') }}</p>
            </div>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
    ...
    <div class="px-4 md:px-6 py-3 md:py-4 border-b border-gray-200">
        <h2 class="text-base md:text-lg font-semibold text-gray-900">{{ __('reports.tab_student_overview') }} ({{ $paginatedRows->total() }})</h2>
    </div>
        ...
        <div x-show="open" x-collapse>
            <form method="GET" action="{{ route($indexRoute, $routeParams) }}" class="px-4 md:px-6 pb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                    {{-- Type Filter --}}
                    <div>
                        <label for="type" class="block text-sm font-medium text-gray-700 mb-1">{{ __('reports.filter_type') }}</label>
                    ...
                    <div>
                        <label for="entity_id" class="block text-sm font-medium text-gray-700 mb-1">{{ __('reports.filter_entity') }}</label>
                    ...
                    {{-- Student Search --}}
                    <div>
                        <label for="student_search" class="block text-sm font-medium text-gray-700 mb-1">{{ __('reports.search_student') }}</label>
            ...
            @foreach($paginatedRows as $row)
                @php
                    ...
                    $metadata = [
                        ...
                        ['icon' => 'ri-pie-chart-line', 'text' => __('reports.attendance_rate') . ': ' . $row->attendance_rate . '%', 'class' => $attendanceColor],
                        ['icon' => 'ri-calendar-check-line', 'text' => __('reports.sessions_completed') . ': ' . $row->sessions_completed],
                    ];

                    if ($showTeacherColumn && !empty($row->teacher_name)) {
                        $metadata[] = ['icon' => 'ri-user-star-line', 'text' => __('reports.teacher_name') . ': ' . $row->teacher_name];
                    }

                    if ($row->avg_performance !== null) {
                        $metadata[] = ['icon' => 'ri-bar-chart-line', 'text' => __('reports.avg_performance_label') . ': ' . $row->avg_performance . '/10'];
                    }
    ...
    @else
        <div class="px-4 md:px-6 py-8 md:py-12 text-center">
            ...
            <h3 class="text-base md:text-lg font-medium text-gray-900 mb-1 md:mb-2">{{ __('reports.empty_students_title') }}</h3>
            <p class="text-sm md:text-base text-gray-600">
                @if($hasActiveFilters)
                    {{ __('reports.empty_students_filter_description') }}
                @else
                    {{ __('reports.empty_students_description') }}
                @endif
            </p>
            @if($hasActiveFilters)
                <a href="{{ route($indexRoute, array_merge($routeParams, ['tab' => 'students'])) }}"
