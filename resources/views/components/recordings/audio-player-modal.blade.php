...
<div
    x-data="{
        ...
        async createWaveform() {
            ...
            const trackUrl = this.waveformUrl || this.audioUrl;
            const peaksUrl = this.peaksUrl;
            const container = this.$refs.waveformContainer;
            if (!trackUrl || !container) return;
            const WaveSurfer = await this.ensureWavesurfer();
            if (!WaveSurfer) return;
            // If a newer track was loaded while we were awaiting the import, bail.
            if (trackUrl !== (this.waveformUrl || this.audioUrl)) return;

            let peaks = null;
            let decodedDuration = 0;

            // Fast path: server-side pre-computed peaks JSON (~10 KB).
            // Generated by the faststart cron on the LiveKit VPS.
            if (peaksUrl) {
                try {
                    ...
                    if (res.ok) {
                        ...
                        if (trackUrl !== (this.waveformUrl || this.audioUrl)) return;
                        if (json && Array.isArray(json.peaks) && json.peaks.length) {
                            peaks = json.peaks;
                            decodedDuration = json.duration || 0;
                        }
                    }
                } catch (e) {
            ...
            // Fallback: fetch the full audio and decode via Web Audio. Used for older
            // recordings that don't yet have a peaks.json (cron picks them up within ~60 s)
            // and for any future deploy where the peaks pipeline is unavailable.
            if (!peaks) {
                try {
                    const response = await fetch(trackUrl, { credentials: 'same-origin' });
                    ...
                    if (trackUrl !== (this.waveformUrl || this.audioUrl)) {
                    ...
                    // Decode failed (CORS, network, unsupported codec). Leave placeholder visible.
                    return;
                }
            }
