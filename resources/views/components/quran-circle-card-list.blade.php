...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 card-hover relative">
  <!-- Card Header -->
  <div class="flex items-start justify-between mb-4">
    ...
    {{-- Status Badge - Show "Enrolled" if enrolled, otherwise show enrollment status --}}
    @if($isEnrolled)
      <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold shadow-sm bg-green-100 text-green-700">
  ...
  @if($isEnrolled)
    <a href="{{ route('quran-circles.show', ['subdomain' => $academy->subdomain ?? 'itqan-academy', 'circleId' => $circle->id]) }}"
       class="min-h-[44px] flex items-center justify-center w-full bg-green-50 text-green-700 border border-green-200 px-4 py-2.5 rounded-lg text-sm font-semibold hover:bg-green-100 transition-colors text-center">
