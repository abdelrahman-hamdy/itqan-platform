...
<script>
    ...
    loadScript('{{ asset("js/livekit/telemetry.js") }}?v={{ filemtime(public_path("js/livekit/telemetry.js")) }}', 'telemetry')
        ...
        .then(() => {
            // Feature modules — whiteboard + mushaf rely on `index.js`
            // topic-aware routing being live, so they load AFTER the core
            // modules. Both are no-ops until window.meeting is initialized
            // by the user clicking start.
            return Promise.all([
...
<script>
    ...
    class AutoAttendanceTracker {
        ...
        async recordLeave() {
            ...
            try {
                const response = await fetchWithAuth('/api/sessions/meeting/leave', {
                    method: 'POST',
                    body: JSON.stringify({
