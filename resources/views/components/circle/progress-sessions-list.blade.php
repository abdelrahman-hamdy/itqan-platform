                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center space-x-4 space-x-reverse">
                            ...
                            <div class="flex flex-col items-center">
                                @if($session->getStatusEnum()->value === 'completed')
                            ...
                            <div class="flex-1">
                                <div class="flex items-center space-x-3 space-x-reverse mb-2">
                                    ...
                                    @if($session->getStatusEnum()->value === 'completed' && ($session->papers_memorized_today || $session->verses_memorized_today))
                                ...
                                <!-- Progress in Session -->
                                @if($session->getStatusEnum()->value === 'completed' && ($session->current_page || $session->current_surah))
                        ...
                        <div class="text-left">
                            <div class="flex flex-col items-end space-y-2">
                                ...
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                                    {{ $session->getStatusEnum()->value === 'completed' ? 
                                       ...
                                       ($session->getStatusEnum()->value === 'scheduled' ? 'bg-blue-100 text-blue-800' : 
                                       ($session->getStatusEnum()->value === 'cancelled' ? 'bg-gray-100 text-gray-800' : 'bg-gray-100 text-gray-800')) }}">
                                    @if($session->getStatusEnum()->value === 'completed')
                                        @if($session->attendance_status === 'attended')
                                ...
                                @if($session->getStatusEnum()->value === 'completed' && ($session->recitation_quality || $session->tajweed_accuracy))
