        ...
        <div class="mb-6" x-data="schedulingPanel()">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                ...
                <div x-show="panelOpen" x-collapse class="px-4 md:px-6 pb-4 md:pb-6">
                    ...
                    <!-- Content: Items Grid + Scheduling Form -->
...
<script>
    ...
    function schedulingPanel() {
        return {
            ...
            teacherId: @js($selectedTeacherId),
            panelOpen: false,
            activeTab: '',
            tabs: @js($tabs),
            items: [],
            ...
            async fetchItems() {
                ...
                try {
                    const response = await fetch(
                        `{{ route('manage.calendar.schedulable-items', ['subdomain' => $subdomain]) }}?tab=${this.activeTab}&teacher_id=${this.teacherId}`,
            ...
            async fetchRecommendations(item) {
                try {
                    const response = await fetch(
                        `{{ $recommendationsRoute }}?item_id=${item.id}&item_type=${item.type}&teacher_id=${this.teacherId}`,
                        { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } }
                    );
                    if (response.ok) {
                        const data = await response.json();
            ...
            async submitSchedule() {
                ...
                try {
                    const response = await fetch(@js(route('manage.calendar.schedule', ['subdomain' => $subdomain])), {
                        method: 'POST',
                        headers: {
                    ...
                    if (response.ok) {
                        ...
                        if (window.supervisorCalendar) {
                            window.supervisorCalendar.refetchEvents();
                        }
                    } else {
    ...
    function sessionDetailModal() {
        return {
            ...
            async show(eventData) {
                ...
                try {
                    const response = await fetch(
                        `{{ $sessionDetailRoute }}?source=${source}&session_id=${sessionId}&teacher_id=${this.teacherId}`,
                        { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } }
                    );
                    const data = await response.json();
                    if (data.success) {
            ...
            async refreshSession() {
                ...
                try {
                    const response = await fetch(
                        `{{ $sessionDetailRoute }}?source=${this.session.source}&session_id=${this.session.id}&teacher_id=${this.teacherId}`,
                        { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } }
                    );
                    const data = await response.json();
                    if (data.success) {
            ...
            async saveEdit() {
                ...
                try {
                    ...
                    const response = await fetch(@js(route('manage.calendar.update-session', ['subdomain' => $subdomain])), {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': @js(csrf_token()),
            ...
            async saveQuranHomework() {
                ...
                try {
                    ...
                    const response = await fetch(@js(route('manage.calendar.quran-homework', ['subdomain' => $subdomain])), {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': @js(csrf_token()),
            ...
            async saveAcademicHomework() {
                ...
                try {
                    ...
                    const response = await fetch(@js(route('manage.calendar.academic-homework', ['subdomain' => $subdomain])), {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': @js(csrf_token()),
