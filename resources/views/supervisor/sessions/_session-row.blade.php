@php
    ...
    $typeConfig = match(true) {
        ...
        $isTrial => ['label' => __('supervisor.sessions.type_quran_trial'), 'icon' => 'ri-gift-line', 'bg' => 'bg-pink-50', 'text' => 'text-pink-600'],
    ...
    if ($isCompleted) {
        ...
        // Individual sessions are auto-counted via session.subscription_counted
        // and that flag may not propagate to meeting_attendances.counts_for_subscription.
    ...
    <td class="px-4 py-3 whitespace-nowrap" @click.stop>
        @if($isCompleted && !$isTrial)
            <div class="space-y-1.5">
                ...
                <div class="flex items-center gap-1.5">
                    <span class="text-[10px] text-gray-400 w-6 shrink-0">{{ __('supervisor.sessions.teacher_short') }}</span>
                    <button type="button"
                ...
                <div class="flex items-center gap-1.5">
                    <span class="text-[10px] text-gray-400 w-6 shrink-0">{{ __('supervisor.sessions.student_short') }}</span>
                    <button type="button"
