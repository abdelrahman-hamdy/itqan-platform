...
<div>
    <x-ui.breadcrumb
        :items="[['label' => __('supervisor.students.page_title')]]"
    ...
    <div class="mb-6 md:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900">{{ __('supervisor.students.page_title') }}</h1>
            <p class="mt-1 md:mt-2 text-sm md:text-base text-gray-600">{{ __('supervisor.students.page_subtitle') }}</p>
        </div>
        @if($isAdmin)
            <a href="{{ route('manage.students.create', ['subdomain' => $subdomain]) }}"
               class="min-h-[44px] inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium whitespace-nowrap cursor-pointer">
                ...
                {{ __('supervisor.students.add_student') }}
            </a>
        @endif
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div class="px-4 md:px-6 py-3 md:py-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-3">
            <h2 class="text-base md:text-lg font-semibold text-gray-900">
                {{ __('supervisor.students.list_title') }} ({{ $students->total() }})
            </h2>
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" type="button"
                    ...
                    <span>
                        @switch($currentSort)
                            @case('name_desc') {{ __('supervisor.students.sort_name_desc') }} @break
                            @case('newest') {{ __('supervisor.students.sort_newest') }} @break
                            @case('oldest') {{ __('supervisor.students.sort_oldest') }} @break
                            @case('grade_level') {{ __('supervisor.students.sort_grade_level') }} @break
                            @default {{ __('supervisor.students.sort_name_asc') }}
                        @endswitch
                    </span>
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    @foreach(['name_asc', 'name_desc', 'newest', 'oldest', 'grade_level'] as $sortOption)
                        <a href="{{ request()->fullUrlWithQuery(['sort' => $sortOption, 'page' => 1]) }}"
                           class="block px-4 py-2 text-sm cursor-pointer {{ $currentSort === $sortOption ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-gray-700 hover:bg-gray-50' }}">
                            {{ __('supervisor.students.sort_' . $sortOption) }}
                        </a>
                    @endforeach
                </div>
            </div>
        ...
        <div x-data="{ open: {{ $hasActiveFilters ? 'true' : 'false' }} }" class="border-b border-gray-200">
            ...
            <div x-show="open" x-collapse>
                <form method="GET" action="{{ route('manage.students.index', ['subdomain' => $subdomain]) }}" class="px-4 md:px-6 pb-4">
                    @if(request('sort'))
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4">
                        <div>
                            ...
                            <input type="text" name="search" id="search" value="{{ request('search') }}"
                                   placeholder="{{ __('supervisor.students.search_placeholder') }}"
                        ...
                        <div>
                            <label for="program" class="block text-sm font-medium text-gray-700 mb-1">{{ __('supervisor.students.filter_program') }}</label>
                            <select name="program" id="program" class="min-h-[44px] w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="">{{ __('supervisor.students.all_programs') }}</option>
                                <option value="quran" {{ request('program') === 'quran' ? 'selected' : '' }}>{{ __('supervisor.students.program_quran') }}</option>
                                <option value="academic" {{ request('program') === 'academic' ? 'selected' : '' }}>{{ __('supervisor.students.program_academic') }}</option>
                                <option value="interactive" {{ request('program') === 'interactive' ? 'selected' : '' }}>{{ __('supervisor.students.program_interactive') }}</option>
                            </select>
                        </div>
                        <div>
                            <label for="gender" class="block text-sm font-medium text-gray-700 mb-1">{{ __('supervisor.students.filter_gender') }}</label>
                            <select name="gender" id="gender" class="min-h-[44px] w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="">{{ __('supervisor.students.all_genders') }}</option>
                        ...
                        <div>
                            <label for="status" class="block text-sm font-medium text-gray-700 mb-1">{{ __('supervisor.students.filter_status') }}</label>
                            <select name="status" id="status" class="min-h-[44px] w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="">{{ __('supervisor.students.all_statuses') }}</option>
                        ...
                        <div>
                            <label for="grade_level" class="block text-sm font-medium text-gray-700 mb-1">{{ __('supervisor.students.filter_grade_level') }}</label>
                            <select name="grade_level" id="grade_level" class="min-h-[44px] w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="">{{ __('supervisor.students.all_grade_levels') }}</option>
                                @foreach($gradeLevels as $gl)
                                    <option value="{{ $gl->id }}" {{ request('grade_level') == $gl->id ? 'selected' : '' }}>{{ $gl->getDisplayName() }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="flex flex-wrap items-center gap-3 mt-4">
                        ...
                        @if($hasActiveFilters)
                            <a href="{{ route('manage.students.index', ['subdomain' => $subdomain]) }}"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs md:text-sm text-gray-600">
                                    ...
                                    @if($student['grade_level'])
                                        <span class="flex items-center gap-1">
                                            <i class="ri-bookmark-line text-amber-500"></i>
                                            {{ $student['grade_level'] }}
                                        </span>
                                    @endif
                                    <span x-data="{ val: @js($student['user']->email) }" class="flex items-center gap-1 cursor-pointer hover:text-blue-600 transition-colors"
                                    ...
                                    @if(($isAdmin || $canResetPasswords) && $student['user']->plain_password)
                                        <span x-data="{ val: @js($student['user']->plain_password) }" class="flex items-center gap-1 cursor-pointer hover:text-blue-600 transition-colors"
                                              x-on:click="navigator.clipboard.writeText(val); $el.querySelector('.copy-icon').classList.add('ri-check-line'); $el.querySelector('.copy-icon').classList.remove('ri-file-copy-line'); setTimeout(() => { $el.querySelector('.copy-icon').classList.remove('ri-check-line'); $el.querySelector('.copy-icon').classList.add('ri-file-copy-line'); }, 1500)"
                        ...
                        <div x-data="{ expanded: false }" class="ms-0 md:ms-14">
                            ...
                            <div x-show="expanded" x-collapse>
                                <div class="flex flex-wrap items-center gap-2">
                                    {{-- View Sessions --}}
                                    <a href="{{ route('manage.sessions.index', ['subdomain' => $subdomain, 'student_id' => $studentId]) }}"
                                    ...
                                    {{-- Message Student --}}
                                    <a href="{{ route('chat.start-with', ['subdomain' => $subdomain, 'user' => $studentId]) }}"
                                    ...
                                    @if($isAdmin || $canManageStudents)
                                        ...
                                        <button type="button"
                                            onclick="window.confirmAction({
                                                title: @js($student['is_active'] ? __('supervisor.teachers.deactivate') : __('supervisor.teachers.activate')),
                                                message: @js($student['is_active'] ? __('supervisor.students.confirm_deactivate') : __('supervisor.students.confirm_activate')),
                                                confirmText: @js($student['is_active'] ? __('supervisor.teachers.deactivate') : __('supervisor.teachers.activate')),
                                                isDangerous: {{ $student['is_active'] ? 'true' : 'false' }},
                                                icon: '{{ $student['is_active'] ? 'ri-pause-circle-line' : 'ri-play-circle-line' }}',
                                                onConfirm: () => document.getElementById('toggle-form-{{ $studentId }}').submit()
                                            })"
                                            ...
                                            <i class="{{ $student['is_active'] ? 'ri-pause-circle-line' : 'ri-play-circle-line' }}"></i>
                                            {{ $student['is_active'] ? __('supervisor.teachers.deactivate') : __('supervisor.teachers.activate') }}
                                        </button>

                                        {{-- Verify Email --}}
                                        @if(!$student['user']->email_verified_at)
                                            <form id="verify-email-form-{{ $studentId }}" method="POST"
                                                  action="{{ route('manage.students.verify-email', ['subdomain' => $subdomain, 'student' => $studentId]) }}">
                                                @csrf
                                            </form>
                                            <button type="button"
                                                onclick="window.confirmAction({
                                                    title: @js(__('supervisor.students.verify_email')),
                                                    message: @js(__('supervisor.students.confirm_verify_email')),
                                                    confirmText: @js(__('supervisor.students.verify_email')),
                                                    isDangerous: false,
                                                    icon: 'ri-mail-check-line',
                                                    onConfirm: () => document.getElementById('verify-email-form-{{ $studentId }}').submit()
                                                })"
                                                ...
                                                {{ __('supervisor.students.verify_email') }}
                                            </button>
                                        @endif

                                        {{-- Reset Password --}}
                                        <button type="button"
                                            onclick="window.dispatchEvent(new CustomEvent('open-modal-reset-password-{{ $studentId }}'))"
                                    ...
                                    @if($isAdmin)
                                        ...
                                        <button type="button"
                                            onclick="window.confirmAction({
                                                ...
                                                onConfirm: () => document.getElementById('delete-form-{{ $studentId }}').submit()
                                            })"
                    ...
                    @if($isAdmin || $canManageStudents)
                        <x-responsive.modal id="reset-password-{{ $studentId }}" :title="__('supervisor.teachers.reset_password')" size="sm">
                            <form id="reset-pw-form-{{ $studentId }}" method="POST" action="{{ route('manage.students.reset-password', ['subdomain' => $subdomain, 'student' => $studentId]) }}"
                                ...
                                @csrf
                                <div class="space-y-4">
                                    <p class="text-sm text-gray-600">{{ __('supervisor.students.reset_password_description', ['name' => $student['user']->name]) }}</p>
                                    <div>
                                        <label for="new_password_{{ $studentId }}" class="block text-sm font-medium text-gray-700 mb-1">{{ __('supervisor.teachers.new_password') }}</label>
                                        <div class="relative">
                                            <input :type="showPass ? 'text' : 'password'" name="new_password" id="new_password_{{ $studentId }}"
                                    ...
                                    <div>
                                        <label for="new_password_confirmation_{{ $studentId }}" class="block text-sm font-medium text-gray-700 mb-1">{{ __('supervisor.teachers.confirm_password') }}</label>
                                        <div class="relative">
                                            <input :type="showConfirm ? 'text' : 'password'" name="new_password_confirmation" id="new_password_confirmation_{{ $studentId }}"
                                                   class="min-h-[44px] w-full px-3 py-2 pe-10 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                                ...
                                <x-slot:footer>
                                    <div class="flex items-center justify-end gap-3">
                                        ...
                                        <button type="submit" form="reset-pw-form-{{ $studentId }}"
        ...
        @else
            ...
            <div class="px-4 md:px-6 py-8 md:py-12 text-center">
                ...
                @if($hasActiveFilters)
                    <h3 class="text-base md:text-lg font-medium text-gray-900 mb-1 md:mb-2">{{ __('supervisor.students.no_results') }}</h3>
                    <p class="text-sm md:text-base text-gray-600">{{ __('supervisor.students.no_results_description') }}</p>
                    <a href="{{ route('manage.students.index', ['subdomain' => $subdomain]) }}"
                ...
                @else
                    <h3 class="text-base md:text-lg font-bold text-gray-900 mb-1 md:mb-2">{{ __('supervisor.students.no_students') }}</h3>
                    <p class="text-gray-600 text-xs md:text-sm">{{ __('supervisor.students.no_students_description') }}</p>
                @endif
            </div>
        @endif
    </div>
