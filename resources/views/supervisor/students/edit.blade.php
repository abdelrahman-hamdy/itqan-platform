...
<div>
    <x-ui.breadcrumb
        :items="[
            ['label' => __('supervisor.students.page_title'), 'url' => route('manage.students.index', ['subdomain' => $subdomain])],
            ['label' => $student->name, 'url' => route('manage.students.show', ['subdomain' => $subdomain, 'student' => $student->id])],
            ['label' => __('supervisor.students.edit_student')],
        ]"
    ...
    <div class="mb-6 md:mb-8">
        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900">{{ __('supervisor.students.edit_title') }}</h1>
        <p class="mt-1 md:mt-2 text-sm md:text-base text-gray-600">{{ __('supervisor.students.edit_subtitle') }}</p>
    </div>
    ...
    <form method="POST"
          action="{{ route('manage.students.update', ['subdomain' => $subdomain, 'student' => $student->id]) }}"
          ...
          x-data="{
              ...
              fileName: '',
              hasImage: false,
              currentAvatarUrl: '{{ $student->avatar ? asset('storage/' . $student->avatar) : '' }}',
              ...
              get defaultAvatarUrl() {
                  ...
                  return this.assetBase + '/' + g + '-student-avatar.png';
              },
              get avatarBgClass() {
                  return 'bg-blue-100';
              },
        ...
        <div class="max-w-3xl mx-auto space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900">{{ __('supervisor.students.personal_info') }}</h3>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label for="gender" class="block text-sm font-medium text-gray-700 mb-1">
                            {{ __('supervisor.students.gender_label') }} <span class="text-red-600">*</span>
                        </label>
                        <select name="gender" id="gender" required x-model="gender"
                            ...
                            <option value="">{{ __('supervisor.students.gender_placeholder') }}</option>
                            <option value="male" {{ old('gender', $profile?->gender) === 'male' ? 'selected' : '' }}>{{ __('supervisor.students.male_student') }}</option>
                            <option value="female" {{ old('gender', $profile?->gender) === 'female' ? 'selected' : '' }}>{{ __('supervisor.students.female_student') }}</option>
                        </select>
                        @error('gender')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        <label for="birth_date" class="block text-sm font-medium text-gray-700 mb-1">
                            {{ __('supervisor.students.birth_date') }}
                        </label>
                        <input type="date" name="birth_date" id="birth_date"
                               value="{{ old('birth_date', $profile?->birth_date?->format('Y-m-d')) }}"
                               class="min-h-[44px] w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('birth_date') border-red-500 @enderror">
                        @error('birth_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900">{{ __('supervisor.students.academic_info') }}</h3>
                </div>

                <div>
                    <label for="grade_level_id" class="block text-sm font-medium text-gray-700 mb-1">
                        {{ __('supervisor.students.grade_level') }} <span class="text-red-600">*</span>
                    </label>
                    <select name="grade_level_id" id="grade_level_id" required
                            class="min-h-[44px] w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('grade_level_id') border-red-500 @enderror">
                        <option value="">{{ __('supervisor.students.all_grade_levels') }}</option>
                        @foreach($gradeLevels as $gradeLevel)
                            <option value="{{ $gradeLevel->id }}"
                                {{ old('grade_level_id', $profile?->grade_level_id) == $gradeLevel->id ? 'selected' : '' }}>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900">{{ __('supervisor.students.parent_info') }}</h3>
                </div>

                <div>
                    <x-forms.phone-input
                        name="parent_phone"
                        label="{{ __('supervisor.students.parent_phone') }}"
                        :required="false"
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('manage.students.show', ['subdomain' => $subdomain, 'student' => $student->id]) }}"
