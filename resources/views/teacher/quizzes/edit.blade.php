<x-layouts.teacher :title="__('teacher.quizzes.edit_title') . ' - ' . $quiz->title . ' - ' . config('app.name')">
@php
...
<div class="max-w-4xl mx-auto">
    ...
    <x-ui.breadcrumb
        :items="[
            ...
            ['label' => $quiz->title, 'route' => route('teacher.quizzes.show', ['subdomain' => $subdomain, 'quiz' => $quiz->id]), 'truncate' => true],
            ['label' => __('teacher.quizzes.edit_title')],
        ]"
        view-type="teacher"
    />
    ...
    {{-- Form --}}
    <form method="POST"
          action="{{ route('teacher.quizzes.update', ['subdomain' => $subdomain, 'quiz' => $quiz->id]) }}"
          x-data="quizForm()"
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm"
                              placeholder="{{ __('teacher.quizzes.field_description_placeholder') }}">{{ old('description', $quiz->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6 mb-4 md:mb-6">
            ...
            <template x-for="(question, qIndex) in questions" :key="qIndex">
                <div class="border border-gray-200 rounded-lg p-4 mb-4 last:mb-0">
                    ...
                    {{-- Hidden ID for existing questions --}}
        ...
        <div class="flex flex-col sm:flex-row justify-end gap-3">
            <a href="{{ route('teacher.quizzes.show', ['subdomain' => $subdomain, 'quiz' => $quiz->id]) }}"
            ...
            <button type="submit"
                ...
                {{ __('teacher.quizzes.update_quiz') }}
            </button>
        </div>
    </form>
</div>
...
function quizForm() {
    return {
        questions: @js($quiz->questions->map(fn($q) => [
            'id' => $q->id,
        ...
        addQuestion() {
            this.questions.push({ id: null, text: '', options: ['', '', '', ''], correctOption: 0 });
        },
