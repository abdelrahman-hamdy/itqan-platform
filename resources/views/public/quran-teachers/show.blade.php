<x-layouts.student
  ...
  <nav class="mb-6">
    <ol class="flex items-center space-x-2 space-x-reverse text-sm text-gray-500">
      <li><a href="{{ route('student.profile', ['subdomain' => auth()->user()->academy->subdomain ?? 'itqan-academy']) }}" class="hover:text-gray-900">الرئيسية</a></li>
      <li>/</li>
      <li><a href="{{ route('student.quran-teachers', ['subdomain' => auth()->user()->academy->subdomain ?? 'itqan-academy']) }}" class="hover:text-gray-900">معلمو القرآن</a></li>
      <li>/</li>
            ...
            @if(auth()->user()->user_type === 'student')
              @if($existingTrialRequest)
                <div class="bg-yellow-50 text-yellow-800 rounded-xl p-4 text-center">
                  ...
                  <p class="text-sm">{{ $existingTrialRequest->getStatusLabelAttribute() }}</p>
                  @if($existingTrialRequest->scheduled_at)
                    <p class="text-xs mt-2">{{ $existingTrialRequest->scheduled_at->format('Y/m/d') }}</p>
                  @endif
                </div>
              @else
              ...
              @endif
            @else
              <div class="flex items-center justify-center gap-2 bg-gray-50 text-gray-500 text-center py-3 rounded-xl text-sm">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($packages as $package)
          <x-teacher.package-card
            ...
            :academy="$academy"
            color="blue"
            :subscribe-route="route('public.quran-teachers.subscribe', ['subdomain' => auth()->user()->academy->subdomain ?? $academy->subdomain, 'teacher' => $teacher->id, 'packageId' => $package->id])"
