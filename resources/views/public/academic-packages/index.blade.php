...
<head>
  ...
  <title>الباقات الأكاديمية - {{ $academy->name ?? 'أكاديمية إتقان' }}</title>
...
<body class="bg-gray-50 font-sans">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-3 space-x-reverse">
          ...
          <div>
            ...
            <p class="text-sm text-gray-600">الباقات الأكاديمية</p>
          </div>
        </div>
  ...
  <section class="bg-gradient-to-br from-primary to-secondary text-white py-16">
    <div class="container mx-auto px-4 text-center">
      <h1 class="text-4xl font-bold mb-4">الباقات الأكاديمية المتميزة</h1>
      <p class="text-xl opacity-90 mb-8">اختر الباقة المناسبة لك واحصل على تعليم أكاديمي متميز مع أفضل المعلمين</p>
    ...
    <div class="container mx-auto px-4">
              ...
              <div class="bg-gradient-to-r from-primary to-secondary text-white p-6">
                <div class="flex items-center justify-between mb-2">
                  ...
                  <span class="bg-white/20 px-3 py-1 rounded-full text-sm">
                    {{ $package->package_type === 'individual' ? 'فردي' : 'جماعي' }}
                  </span>
                </div>
                ...
                <div class="space-y-3 mb-6">
                  ...
                  @if($package->features)
                    @foreach($package->features as $feature)
                      <div class="flex items-center text-sm">
                        <i class="ri-star-line text-yellow-500 ml-2"></i>
                        <span>{{ $feature }}</span>
                      </div>
                    @endforeach
                  @endif
      ...
      @else
        <div class="text-center py-16">
          ...
          <h3 class="text-2xl font-bold text-gray-700 mb-2">لا توجد باقات متاحة حالياً</h3>
          <p class="text-gray-600">سيتم إضافة باقات أكاديمية قريباً</p>
        </div>
      @endif
      ...
      @if($teachers->count() > 0)
        <div class="mt-16">
          <div class="text-center mb-12">
            <h2 class="text-3xl font-bold text-gray-900 mb-4">المعلمون الأكاديميون</h2>
            <p class="text-gray-600 text-lg">تعرف على نخبة من أفضل المعلمين المعتمدين</p>
          </div>
            ...
            @foreach($teachers->take(6) as $teacher)
              <div class="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 p-6 border border-gray-200">
                ...
                <div class="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <h3 class="font-bold text-gray-900">{{ $teacher->user->name ?? 'المعلم' }}</h3>
                    <p class="text-gray-600 text-sm">معلم أكاديمي معتمد</p>
                    @if($teacher->experience_years)
                      <p class="text-gray-500 text-xs">{{ $teacher->experience_years }} سنوات خبرة</p>
                    @endif
                  </div>
                </div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
      <div class="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-200">
          <div class="flex items-center justify-between">
            <h3 class="text-xl font-bold text-gray-900">اختر المعلم المناسب</h3>
        ...
        fetch(`/{{ $academy->subdomain }}/api/academic-packages/${packageId}/teachers`)
          ...
          .then(data => {
            ...
            if (data.teachers && data.teachers.length > 0) {
              ...
              modalContent.innerHTML = '<p class="text-center text-gray-600">لا يوجد معلمون متاحون لهذه الباقة حالياً</p>';
            }
