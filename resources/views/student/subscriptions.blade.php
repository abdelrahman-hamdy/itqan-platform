@php
    ...
    foreach ($individualQuranSubscriptions as $sub) {
        ...
        $allSubscriptions->push([
            ...
            'currency' => $sub->currency ?? $academy->currency?->value ?? 'EGP',
...
<x-layouts.authenticated
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                            ...
                            <div class="flex flex-wrap items-center gap-2 sm:flex-shrink-0">
                                ...
                                @if(!$isParent && ($subscription['can_delete'] ?? false))
                                    <button type="button"
                                            onclick="deleteSubscription('{{ $subscription['model_type'] }}', {{ $subscription['id'] }})"
                                            class="inline-flex items-center justify-center min-h-[44px] px-4 py-2 bg-gray-50 text-gray-700 border border-gray-300 rounded-xl md:rounded-lg text-sm font-medium hover:bg-gray-100 transition-colors">
                    ...
                    <div class="bg-gradient-to-l from-amber-500 to-orange-500 p-4">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium
                                {{ $trial->status === \App\Enums\TrialRequestStatus::SCHEDULED ? 'bg-blue-100 text-blue-800' :
                                   ($trial->status === \App\Enums\TrialRequestStatus::PENDING ? 'bg-yellow-100 text-yellow-800' :
                                   ($trial->status === \App\Enums\TrialRequestStatus::COMPLETED ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800')) }}">
                                {{ $trial->status->label() }}
                            </span>
                        </div>
                    </div>
    ...
    <script>
        ...
        function deleteSubscription(type, id) {
            showModal(
                'bg-gray-100',
                'ri-delete-bin-line text-gray-600 text-3xl',
                '{{ __("student.confirm.delete_subscription_title") }}',
                '{{ __("student.confirm.delete_subscription_message") }}',
                'bg-gray-700 text-white hover:bg-gray-800',
                () => {
                    const form = document.createElement('form');
                    form.method = 'POST';
                    form.action = deleteSubscriptionUrl.replace('__TYPE__', type).replace('__ID__', id);
                    form.innerHTML = `
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
