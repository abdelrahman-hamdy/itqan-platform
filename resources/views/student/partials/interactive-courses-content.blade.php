    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-1 md:mb-2">
          {{ __('student.interactive_courses.title') }}
        </h1>
        <p class="text-sm md:text-base text-gray-600">
          {{ __('student.interactive_courses.description') }}
        </p>
      </div>
      @auth
      <div class="bg-white rounded-xl px-4 md:px-6 py-2.5 md:py-3 border border-gray-200 shadow-sm flex-shrink-0">
        <span class="text-xs md:text-sm text-gray-600">{{ __('student.interactive_courses.my_active_courses') }} </span>
  ...
  <div class="mb-4 md:mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
    <p class="text-sm md:text-base text-gray-600">
      ...
      {{ __('student.interactive_courses.available_courses') }}
    </p>
    @if($courses->total() > 0)
    <p class="text-xs md:text-sm text-gray-500">
      {{ __('student.interactive_courses.showing_results') }} {{ $courses->firstItem() }} - {{ $courses->lastItem() }} {{ __('student.interactive_courses.of_total') }} {{ $courses->total() }}
    </p>
    @endif
  </div>
  ...
  <div class="text-center py-8 md:py-12 bg-white rounded-xl border border-gray-200">
    ...
    <h3 class="text-base md:text-lg font-semibold text-gray-900 mb-2">{{ __('student.interactive_courses.no_courses_title') }}</h3>
    <p class="text-sm md:text-base text-gray-600 mb-4 md:mb-6 px-4">{{ __('student.interactive_courses.no_results_description') }}</p>
