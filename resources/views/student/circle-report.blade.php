<x-layouts.student
    :title="'تقريري - ' . config('app.name', 'منصة إتقان')"
    ...
    <nav class="mb-8">
        <ol class="flex items-center space-x-2 space-x-reverse text-sm text-gray-600">
            ...
            @if($isQuran)
                ...
                @else
                    <li><a href="{{ route('student.group-circles.show', ['subdomain' => auth()->user()->academy->subdomain ?? 'itqan-academy', 'circle' => $circle->id]) }}" class="hover:text-primary">{{ $circle->name }}</a></li>
                @endif
            @elseif($isAcademic)
        ...
        <div class="flex items-center justify-between">
            <div class="flex-1">
                ...
                <div class="mt-4 flex flex-wrap gap-6 text-sm text-blue-100">
                    ...
                    @elseif($isInteractive && isset($course))
                        @if($course->start_date)
                        <div class="flex items-center gap-2">
                            <i class="ri-calendar-line"></i>
                            <span>تاريخ البداية: {{ $course->start_date->format('Y-m-d') }}</span>
                        </div>
                        @endif
                        <div class="flex items-center gap-2">
                            <i class="ri-file-list-line"></i>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        @if($isQuran)
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600">تقييمي العام</p>
        ...
        @else
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600">متوسط أدائي</p>
