<x-student title="{{ auth()->user()->academy->name ?? 'أكاديمية إتقان' }} - {{ $course->title }}">
  <x-slot name="description">{{ $course->description }} - {{ auth()->user()->academy->name ?? 'أكاديمية إتقان' }}</x-slot>
