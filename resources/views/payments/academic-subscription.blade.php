{{-- Minimal gateway selection page for academic subscription payment retry --}}
...
<x-layouts.authenticated role="student"
    title="{{ __('payments.academic_payment.page_title') }} - {{ $academy->name ?? '' }}">
    ...
    <div class="max-w-lg mx-auto py-12">
        ...
        <form id="gateway-form" method="POST"
              action="{{ route('academic.subscription.payment.submit', ['subdomain' => $academy->subdomain, 'subscription' => $subscription->id]) }}"
              style="display: none;">
            @csrf
